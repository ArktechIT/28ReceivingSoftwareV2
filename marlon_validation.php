<?php
    ...
    function response($resp, $poContentId, $ptag, $lot, $supplier = '', $customer = '')
    {
        echo json_encode(array(
                            "resp"=> $resp, 
                            "poContentId"=> $poContentId, 
                            "PTAG" => $ptag, 
                            "lot" => $lot, 
                            "supplier" => $supplier, 
                            "customer" => $customer
    ...
    if(isset($_GET['itemTag']))
    {
        ...
        $sql = "SELECT 
                ...
                partId
                FROM ppic_lotlist 
                WHERE lotNumber = '$itemTags' 
                OR productionTag = '$itemTags'";
        ...
        if (mysqli_num_rows ($filteredData) != 0)
        {
            ...
            $sql2 = "SELECT 
                    ...
                    FROM ppic_workschedule 
                    WHERE lotNumber = '".$row['lotNum']."' 
                    AND (processCode = 137 OR processCode = 138)";
            ...
            $sql = "SELECT 
                    ...
                    FROM ppic_workschedule 
                    WHERE lotNumber = '".$row['lotNum']."' 
                    AND status = 0 AND (processCode = 137 
                    OR processCode = 138) 
            ...
            $sql = "SELECT *, 
                    CASE 
                        WHEN identifier=1 THEN GROUP_CONCAT(poContentId)
                        WHEN identifier=4 THEN poId 
                    END AS pOrder
                    FROM ppic_lotlist
                    WHERE lotNumber = '".$row['lotNum']."'";
            ...
            if($row['identifier'] == 1)
            {
                ...
                while($number>=0)
                {
                    $sqlPo = "SELECT 
                              ...
                              WHERE dataThree = '$remarksArray[$number]' 
                              AND poContentId 
                              IN ('".implode("','",$poContentIdArray)."')";
                    $checkPO = mysqli_query($connection, $sqlPo);
                    $rowPO = mysqli_fetch_array($checkPO);

                    $sql = "SELECT customerId FROM cadcam_parts WHERE partId = '".$row['partId']."'";
                    $checkCustomer = mysqli_query($connection, $sql);
                    $customerResult = mysqli_fetch_array($checkCustomer);
                    ...
                    else
                    {
                        ...
                        {
                            response(
                                "NO PURCHASE ORDER<br>".$remarksContent, 
                                "none", 
                                $row['productionTag'], 
                                $row['lotNum']
                            );
